...
from src.Optimisation import Sensitivity, GradOptimiser
...
class WLTC:
    ...
    def __init__(self, t, v, m, Cd, A, Crr):
    ...
    def evaluate_cost(self):
        ...
        for i in range(len(self.t) - 1):
            ...
            v_avg = (self.v[i + 1] + self.v[i]) / 2

            # Inertial
            dp_dt = ((self.v[i + 1] - self.v[i]) / dt) * self.m[0]
            if dp_dt < 0:
                dp_dt = 0

            # Aerodynamic
            F_a = 0.5 * 1.225 * self.A[0] * self.Cd[0] * v_avg * v_avg

            # Rolling resistance
            F_rr = self.m[0] * 9.81 * self.Crr[0]
